                                ...
                                <div class="flex justify-center">
                                  ...
                                  <div id="riwayat-modal-{{ $item->id }}" class="hidden fixed inset-0 bg-gray-900 bg-opacity-50 flex justify-center items-center z-50">
                                    <div class="bg-white rounded-lg shadow-lg w-1/2 p-6">
                                        ...
                                        @if ($riwayat2 && $riwayat2->periksa)
                                        <table class="">
                                          <tr>
                                            <td class="border-2 p-2">Tanggal Periksa</td>
                                            <td class="border-2 p-2">: {{$riwayat2->periksa->tgl_periksa}}</td>
                                          </tr>
                                          <tr>
                                            <td class="border-2 p-2">Catatan</td>
                                            <td class="border-2 p-2">: {{$riwayat2->periksa->catatan}}</td>
                                          </tr>
                                          <tr>
                                            <td class="border-2 p-2">Daftar Obat</td>
                                                  @if ($item->periksa)
                                                      @foreach ($item->periksa->detailPeriksa as $detail)
                                                              <td class="border-2 p-2 flex">{{ $detail->obat->nama_obat ?? '-' }}</td>
                                                      @endforeach
                                                  @endif
                                          </tr>
                                          <tr>
                                            <td class="border-2 p-2">Biaya Periksa</td>
                                            <td class="border-2 p-2">: {{$riwayat2->periksa->biaya_periksa}}</td>
                                          </tr>
                                        </table>
                                        @else
                                            <p>Belum ada data pemeriksaan.</p>
                                        @endif
                                        <button data-modal-hide="riwayat-modal-{{ $item->id }}" class="bg-green-500 text-white px-4 py-2 rounded mt-4">Tutup</button>
                                    </div>
                                  </div>
