...
class PasienController extends Controller
{
    ...
    public function daftarPoli()
    {
        ...
        $riwayat2 = Daftar_poli::with([
            'pasien',
            'jadwalPeriksa.dokter.poli',
            'periksa' => function($query) {
                $query->with([
                    'detailPeriksa' => function($subQuery) {
                        $subQuery->with('obat'); // Ambil data obat yang berelasi dengan detailPeriksa
                    }
                ]); // Asumsikan ada relasi 'detailPeriksa' di model Periksa
            }
        ])->where('id_pasien', $idPasien)->first();

        return view('pasien.daftar', compact('poli','riwayat','riwayat2'));
